...
describe('obj.intersection', () => {
	it("should require at least one argument", () => {
		_assert.throws(() => { _obj.intersection() }, TypeError);
	});

	it("should require all arguments to be objects", () => {
		_assert.throws(() => { _obj.intersection({}, null) }, TypeError);
	});

	it("should create a copy of a single argument", () => {
		const op1 = { a : 1, b : 2 };

		_assert.deepStrictEqual(_obj.intersection(op1), op1);
		_assert.notEqual(_obj.intersection(op1), op1);
	});
	...
	it("should create a shallow object intersection", () => {
		...
		_assert.deepStrictEqual(_obj.intersection(op1, op2, op3), res);
	});
	...
	it("should treat non-intersecting objects as empty", () => {
		...
		_assert.deepStrictEqual(_obj.intersection(op1, op2), res);
	});
	...
	it("should treat [ a ∩ b ∩ c ] as [ a ∩ (b ∩ c) ]", () => {
		...
		_assert.deepStrictEqual(_obj.intersection(op1, op2, op3), _obj.intersection(op1, _obj.intersection(op2, op3)));
	});
